...
| `data/snapshots/<YYYY-MM-DD>[-N]/schedule.json` | stats-loader | The season's NFL schedule (Sleeper `/schedule/nfl/regular/<season>`), verbatim. Optional — snapshots predating it still load. Feeds the Opportunity Forecast (`context`) model's opponent lookups. |
